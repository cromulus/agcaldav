  ...
  class Client
    ...
    def find_events data
      ...
      __create_http.start {|http|
        ...
        req.body = AgCalDAV::Request::ReportVEVENT.new(
          param_time_parse(data[:start]),
          param_time_parse(data[:end])
        ).to_xml
    ...
    def param_time_parse(timestamp)
      if timestamp.is_a? Integer
        Time.at(timestamp).utc.strftime("%Y%m%dT%H%M%S")
      else
        d=DateTime.parse(timestamp).new_offset("0")
